...
Мне очень нравится модульная музыка, поэтому не могу не порекомендовать [modarchive](https://modarchive.org/). Треки в формате [XM](https://en.wikipedia.org/wiki/XM_(file_format)) можно [воспроизводить в игре напрямую](https://github.com/quasilyte/xm). Большую часть остальных форматов (IT, MOD, S3M) можно конвертировать в XM через [MilkyTracker](https://milkytracker.org/).
